...
function zoomMap(group){const points=group.map(x=>mapProject(x.place)),xs=points.map(p=>p.x),ys=points.map(p=>p.y),cx=(Math.min(...xs)+Math.max(...xs))/2,cy=(Math.min(...ys)+Math.max(...ys))/2,width=Math.max(homeView.width*.28,Math.max(...xs)-Math.min(...xs)+160,(Math.max(...ys)-Math.min(...ys)+130)*mapAspect);mapFocus=clampMapView({left:cx-width/2,top:cy-width/mapAspect/2,width});renderMap()}
...
document.querySelector('.map-image').addEventListener('wheel',event=>{event.preventDefault();const rect=event.currentTarget.getBoundingClientRect(),x=Math.min(1,Math.max(0,(event.clientX-rect.left)/rect.width)),y=Math.min(1,Math.max(0,(event.clientY-rect.top)/rect.height)),bounds=mapFocus||homeView,factor=event.deltaY<0?.82:1.22,width=Math.min(homeView.width,Math.max(homeView.width*.28,bounds.width*factor)),height=width/mapAspect,cx=bounds.left+x*bounds.width,cy=bounds.top+y*bounds.height;mapFocus=width>=homeView.width*.995?null:clampMapView({left:cx-x*width,top:cy-y*height,width});renderMap()},{passive:false});
