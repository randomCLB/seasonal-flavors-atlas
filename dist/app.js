...
function detailHtml(f){
  ...
  ${f.image?`<figure class="detail-photo"><img src="${f.image}" alt="${f.imageAlt}" fetchpriority="high"><figcaption>${f.imageCaption||f.cardNote||f.region}</figcaption></figure>`:''}</div>
  ${(f.images||[]).length?`<div class="detail-gallery">${(f.images||[]).map(p=>`<figure><img src="${p.src}" alt="${p.alt}" loading="lazy"><figcaption>${p.caption||p.alt}</figcaption></figure>`).join('')}</div>`:''}
  ...
  <section><p class="detail-num">02 / 何时遇见</p><h2>${seasonHeading}</h2><p>${f.season}</p>${f.place?`<p>${f.place}</p>`:''}${f.context?`<p>${f.context}</p>`:''}${(f.articleImages||[]).map(p=>`<figure class="article-photo"><img src="${p.src}" alt="${p.alt}" loading="lazy"><figcaption>${p.caption||p.alt}</figcaption></figure>`).join('')}</section>
...
function renderSearch(query=''){
  ...
  $('searchResults').innerHTML=matches.length?matches.map(f=>`<button data-food="${f.id}"><span><strong>${f.name}</strong><small>${f.alias.join(' · ')}</small></span><em>${categoryNames[f.category]} · ${f.region} ↗</em></button>`).join(''):'<p class="search-empty">还没有找到这味食材，试试它的别名。</p>';
